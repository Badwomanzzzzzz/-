      ...
      <table class="courses-table">
        ...
        <tbody>
          <tr v-for="course in courses" :key="course.CourseID">
            ...
            <td>
              <button class="edit-button" @click="handleEdit(course)">编辑</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div class="dialog" v-if="dialogVisible">
      ...
      <div class="dialog-content">
        <div class="dialog-header">
          <h3>{{ dialogType === 'add' ? '添加课程' : '编辑课程' }}</h3>
...
const loadCourses = async () => {
  try {
    const response = await api.get('/basic-info/courses');
...
const handleSubmit = async () => {
  try {
    if (dialogType.value === 'add') {
      await api.post('/basic-info/courses', form.value);
      alert('添加成功');
    } else {
      // 暂时只支持添加，编辑功能待实现
      alert('编辑功能待实现');
